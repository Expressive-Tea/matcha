...
fn add_errors_when_multiple_controllers_and_writes_nothing() {
    ...
    assert_eq!(std::fs::read_to_string(cdir.join("a.controller.ts")).unwrap(), original);
    assert_eq!(std::fs::read_to_string(cdir.join("b.controller.ts")).unwrap(), original);
}
